...
import { MiddlewareFunction, RouteMap } from "../core/type.js";
...
export class RouteManager extends AddRoutePathAbc implements RouteManagerI {
	...
	@bound controllerHandler(req: IncomingMessage, res: ServerResponse): void {
		...
		const newResponse: ArgumentedServerResponseInterface =
			ArgumentResponse(res);
		...
		const handler: RouteMap | undefined = this.#getHandler(
			url,
			isDynamic,
			isStatic,
		);
		...
		this.#middlewareManger.run(newRequest, newResponse, (_, __, next) => {
			if (!this.#validateRoute(handler, method, newRequest, newResponse))
				return;
			...
			const callbacks: MiddlewareFunction[] =
				handler!.get(method!)?.middlewares ?? [];
